...
def _calcular_metricas_dashboard(df: pd.DataFrame) -> dict[str, float | int]:
    ...
    if df_probabilidades.empty:
        ...
        qtd_baixa_confianca = int((df_probabilidades["Confianca"] < LIMIAR_BAIXA_CONFIANCA).sum())
...
def _render_visao_geral(metricas: dict[str, float | int]) -> None:
    ...
    with m2:
        _render_metric_card("Com rachaduras", metricas["qtd_rachado"], f"{metricas['perc_rachado']:.1f}%")

    with m3:
        _render_metric_card("Bom estado", metricas["qtd_bom"], f"{metricas['perc_bom']:.1f}%")

    with m4:
        _render_metric_card("Confiança média", f"{metricas['confianca_media']:.1%}")
...
def _render_exportacao_dashboard(df: pd.DataFrame, metricas: dict[str, float | int]) -> None:
    st.markdown('<div class="section-title">📦 Exportação do Dashboard</div>', unsafe_allow_html=True)
...
def _render_indicadores_visuais(df: pd.DataFrame, metricas: dict[str, float | int]) -> None:
    ...
    with g1:
        df_resultados = pd.DataFrame({
            ...
            "Percentual": [
                metricas["qtd_rachado"] / metricas["total_analises"] if metricas["total_analises"] else 0,
                metricas["qtd_bom"] / metricas["total_analises"] if metricas["total_analises"] else 0,
            ]
        })
...
def _render_historico_detalhado(analises: list[Any]) -> None:
    ...
    for analise in analises:
        ...
        with st.container(border=True):
            st.markdown(f'<div class="file-name">{analise.nome_arquivo}</div>', unsafe_allow_html=True)
